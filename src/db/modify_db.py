...
from src.utils.salary_utils import (
    calculate_min_max,
...
def removeprefix_column(column_name: str, prefix: str) -> None:
    ...
    cur.execute(
        f"UPDATE job_offers SET {column_name} = CASE WHEN {column_name} LIKE '{prefix} %' THEN substring({column_name} FROM length('{prefix} ') + 1) ELSE {column_name} END;"
    )
    conn.commit()
...
def add_prefix_column(column_name: str, old_prefix: str, new_prefix: str) -> None:
    ...
    cur.execute(
        f"UPDATE job_offers SET {column_name} = CASE WHEN {column_name} LIKE '{old_prefix}%' THEN '{new_prefix}' || substring({column_name} FROM length('{old_prefix} ') + 1) ELSE {column_name} END;"
    )
    conn.commit()
...
def update_column_with_function(column_name: str, func: Callable[[Any], Any]) -> None:
    ...
    try:
        ...
        select_query = sql.SQL("SELECT id, {} FROM job_offers").format(
        ...
        for row_id, value in rows:
            ...
            update_query = sql.SQL(
                "UPDATE job_offers SET {} = %s WHERE id = %s"
...
def update_rows_with_function(
    ...
    Args:
        ...
        limit: Optional limit on the number of rows to select/update
    """
    conn = psycopg2.connect(**DB_CONFIG)
    cur = conn.cursor()
    ...
    try:
        ...
        columns_sql = sql.SQL(", ").join(
            sql.Identifier(col) for col in columns_to_fetch
        )
        ...
        if limit is not None:
            select_query = sql.SQL("{} LIMIT {}").format(
                select_query, sql.Literal(limit)
            )
        ...
        logger.info(
            f"update_rows_with_function({column_to_update}): "
...
def populate_categories() -> None:
    """Populate category column using the saved HTML body and strategy parsing."""
    from bs4 import BeautifulSoup
    from src.scraping.scrape_job import get_strategy

    def transform(row: tuple) -> str | None:
        row_id, link, body, category = row

        # If category is already populated (not empty/null/Other), skip
        if category and category != "" and category != "Other":
            return None
        ...
        try:
            ...
            strategy = get_strategy(link)
            new_category = strategy.get_category(soup)

            # If the strategy extracted a category and it's different/more specific, return it
            if new_category and new_category != category:
                print(
                    f"Updating ID {row_id} category from '{category}' to '{new_category}' (link: {link})"
                )
                return new_category
        except Exception as e:
            logger.error(f"Error parsing category for ID {row_id} (link {link}): {e}")

        return None

    update_rows_with_function(
        columns_to_fetch=["id", "link", "body", "category"],
        column_to_update="category",
        transform_func=transform,
        where_clause="category is null or category = '' or category = 'Other'",
        # limit=1,
    )
