...
class PracujStrategy(BaseJobStrategy):
    def get_title(self, soup: BeautifulSoup) -> str:
        title = get_text_helper(
            soup.find("h1", attrs={"data-test": "text-positionName"})
        )
        if title:
            return title
        return ""
    ...
    def get_company(self, soup: BeautifulSoup) -> str:
        return get_text_helper(
            soup.find("h2", attrs={"data-test": "text-employerName"}), ["O firmie"]
        )
    ...
    def get_category(self, soup: BeautifulSoup) -> str:
        ...
        it_specializations = get_text_helper(
            soup.find(attrs={"data-test": "it-specializations"})
        )
    ...
    def get_responsibilities(self, soup: BeautifulSoup) -> str:
        responsibilities = soup.find(
            attrs={"data-test": "section-responsibilities"}
        ).find_all("li")
        return ", ".join(
            get_text_helper(responsibility) for responsibility in responsibilities
        )

    def get_requirements(self, soup: BeautifulSoup) -> str:
        requirements = soup.find(attrs={"data-test": "section-requirements"}).find_all(
            "li"
        )
